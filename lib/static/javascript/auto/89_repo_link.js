...
/******** Javascript for satelite *******/
  ...
  jQuery(document).ready(function(){  
	...
	var this_id = window.location.pathname.replace(/\//g,'').replace(/ideprint/,'');

    if(jQuery.url().param('eprintid') !== undefined){
        this_id = jQuery.url().param('eprintid');
    }
	    console.log(this_id);
       ...
       jQuery.ajax({
	    ...
	    jQuery("div.repo_links").html('<h3>Related publication(s)</h3><ul></ul>');
